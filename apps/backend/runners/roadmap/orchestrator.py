...
class RoadmapOrchestrator:
    ...
    def __init__(
        ...
        # Default output to project's .juict-agentic-os directory (installed instance)
        # Note: auto-claude/ is source code, .juict-agentic-os/ is the installed instance
        if output_dir:
            self.output_dir = Path(output_dir)
        else:
            # Initialize .juict-agentic-os directory and ensure it's in .gitignore
            init_auto_claude_dir(self.project_dir)
            self.output_dir = self.project_dir / ".juict-agentic-os" / "roadmap"
