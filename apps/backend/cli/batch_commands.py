...
def handle_batch_create_command(batch_file: str, project_dir: str) -> bool:
    ...
    specs_dir = Path(project_dir) / ".juict-agentic-os" / "specs"
...
def handle_batch_cleanup_command(project_dir: str, dry_run: bool = True) -> bool:
    ...
    """
    specs_dir = Path(project_dir) / ".juict-agentic-os" / "specs"
    worktrees_dir = Path(project_dir) / ".juict-agentic-os" / "worktrees" / "tasks"
    ...
    if dry_run:
        ...
        for spec_name in completed:
            ...
            if wt_path.exists():
                print(f"    └─ .juict-agentic-os/worktrees/tasks/{spec_name}/")
        print()
