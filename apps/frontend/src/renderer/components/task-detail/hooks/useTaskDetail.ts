...
export function useTaskDetail({ task }: UseTaskDetailOptions) {
  ...
  const [activeTab, setActiveTab] = useState(task.status === 'brainstorming' ? 'pipeline' : 'overview');
  ...
  // isActiveTask: task is currently being processed
  const isActiveTask = task.status === 'executing';
  const needsReview = task.status === 'verifying';
