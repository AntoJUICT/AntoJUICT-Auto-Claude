...
function setupTestDirs(): void {
  ...
  TEST_SPEC_DIR = path.join(TEST_PROJECT_PATH, '.juict-agentic-os/specs/001-test-feature');
...
describe('Task Lifecycle Integration', () => {
  ...
  describe('Spec completion to subtask loading', () => {
    ...
    it('should emit task:statusChange event when task transitions from planning to spec_complete', async () => {
      ...
      if (eventHandler) {
        eventHandler({}, 'task-001', 'plan_review', undefined);
      }
      ...
      expect(callback).toHaveBeenCalledWith('task-001', 'plan_review', undefined);
    });
