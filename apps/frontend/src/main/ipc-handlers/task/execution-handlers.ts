    ...
    async (
      ...
      if (approved) {
        ...
        if (hasWorktree) {
          ...
          // IMPORTANT: Exclude .juict-agentic-os directory to preserve specs and worktree data
          const cleanResult = spawnSync(getToolPath('git'), ['clean', '-fd', '-e', '.juict-agentic-os'], {
            cwd: project.path,
            encoding: 'utf-8',
            stdio: 'pipe',
            env: getIsolatedGitEnv()
          ...
          if (cleanResult.status === 0) {
            console.log('[TASK_REVIEW] Cleaned untracked files in main (excluding .juict-agentic-os)');
          }
