...
function getEffectiveBaseBranch(projectPath: string, specId: string, projectMainBranch?: string): string {
  ...
  const specDir = path.join(projectPath, '.juict-agentic-os', 'specs', specId);
...
export function registerWorktreeHandlers(
   ...
   * Per-spec architecture: Each spec has its own worktree at .juict-agentic-os/worktrees/tasks/{spec-name}/
   */
  ipcMain.handle(
    ...
    async (_, taskId: string): Promise<IPCResult<WorktreeStatus>> => {
      try {
        ...
        // Find worktree at .juict-agentic-os/worktrees/tasks/{spec-name}/
  ...
  ipcMain.handle(
    ...
    async (_, taskId: string, options?: { noCommit?: boolean }): Promise<IPCResult<WorktreeMergeResult>> => {
      ...
      try {
        ...
        const specDir = path.join(project.path, project.autoBuildPath || '.juict-agentic-os', 'specs', task.specId);
        ...
        return new Promise((resolve) => {
          ...
          const handleProcessExit = async (code: number | null, signal: string | null = null) => {
            ...
            if (code === 0) {
              ...
              if (worktreePath) {
                const worktreeSpecDir = path.join(worktreePath, project.autoBuildPath || '.juict-agentic-os', 'specs', task.specId);
...
// Validate project.path
        ...
        const mainSpecsDir = path.join(project.path, '.juict-agentic-os', 'specs');
