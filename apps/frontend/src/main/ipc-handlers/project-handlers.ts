...
export function registerProjectHandlers(
  ...
  ipcMain.handle(
    ...
    async (): Promise<IPCResult<Project[]>> => {
      // Validate that .juict-agentic-os folders still exist for all projects
      ...
      if (resetIds.length > 0) {
        console.warn('[IPC] PROJECT_LIST: Detected missing .juict-agentic-os folders for', resetIds.length, 'project(s)');
      }
    ...
    async (_, projectId: string): Promise<IPCResult<InitializationResult>> => {
      try {
        ...
        if (result.success) {
          ...
          projectStore.updateAutoBuildPath(projectId, '.juict-agentic-os');
        }
  ...
  // Version tracking for .juict-agentic-os is removed since it only contains data
  ipcMain.handle(
      ...
      try {
        ...
        return {
          ...
          data: {
            ...
            updateAvailable: false // No updates for .juict-agentic-os - it's just data
          }
        };
      } catch (error) {
        return {
