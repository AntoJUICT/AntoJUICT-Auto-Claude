...
function getDefaultBranch(projectPath: string): string {
  ...
  const envPath = path.join(projectPath, '.juict-agentic-os', '.env');
...
function loadDependencyConfigs(projectPath: string): DependencyConfig[] {
  const indexPath = path.join(projectPath, '.juict-agentic-os', 'project_index.json');
 ...
 * - .juict-agentic-os/worktrees/terminal/*
 * - .juict-agentic-os/worktrees/tasks/*
 * - .juict-agentic-os/worktrees/pr/*
 */
async function listOtherWorktrees(projectPath: string): Promise<OtherWorktreeInfo[]> {
  ...
  const excludePrefixes = [
    path.join(normalizedProjectPath, '.juict-agentic-os', 'worktrees', 'terminal'),
    path.join(normalizedProjectPath, '.juict-agentic-os', 'worktrees', 'tasks'),
    path.join(normalizedProjectPath, '.juict-agentic-os', 'worktrees', 'pr'),
  ];
