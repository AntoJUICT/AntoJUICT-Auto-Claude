...
describe("IPC Handlers", { timeout: 30000 }, () => {
  ...
  describe("task:list handler", () => {
    ...
    it("should return tasks when specs exist", async () => {
      ...
      // Create .juict-agentic-os directory first (before adding project so it gets detected)
      mkdirSync(path.join(TEST_PROJECT_PATH, ".juict-agentic-os", "specs"), { recursive: true });

      // Add a project - it will detect .juict-agentic-os
      const addResult = await ipcMain.invokeHandler("project:add", {}, TEST_PROJECT_PATH);
      const projectId = (addResult as { data: { id: string } }).data.id;

      // Create a spec directory with implementation plan in .juict-agentic-os/specs
      const specDir = path.join(TEST_PROJECT_PATH, ".juict-agentic-os", "specs", "001-test-feature");
  ...
  describe("Agent Manager event forwarding", () => {
    ...
    it("should forward exit events with status change on failure", async () => {
      ...
      const specDir = path.join(TEST_PROJECT_PATH, ".juict-agentic-os", "specs", "task-1");
      mkdirSync(specDir, { recursive: true });
      writeFileSync(
        path.join(specDir, "implementation_plan.json"),
