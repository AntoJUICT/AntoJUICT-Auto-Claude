...
describe('ProjectStore', () => {
  ...
  describe('addProject', () => {
    ...
    it('should detect auto-claude directory if present', async () => {
      // Create .juict-agentic-os directory (the data directory, not source code)
      mkdirSync(path.join(TEST_PROJECT_PATH, '.juict-agentic-os'), { recursive: true });
      ...
      expect(project.autoBuildPath).toBe('.juict-agentic-os');
    });
  ...
  describe('getTasks', () => {
    ...
    it('should read tasks from filesystem correctly', async () => {
      // Create spec directory structure in .juict-agentic-os (the data directory)
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '001-test-feature');
    ...
    it('should determine status as backlog when no subtasks completed', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '002-pending');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as preview when all subtasks completed', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '003-complete');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as preview when plan status is preview (user review)', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '004-rejected');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine reviewReason from plan when status is human_review', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '005-approved');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should determine status as done when plan status is explicitly done', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '006-done');
      mkdirSync(specsDir, { recursive: true });
  ...
  describe('archiveTasks - multi-location handling', () => {
    it('should archive task from main specs directory only', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '001-test-task');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should archive task from BOTH main and worktree locations', async () => {
      ...
      const mainSpecsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '002-multi-location');
      mkdirSync(mainSpecsDir, { recursive: true });

      // Create spec directory in worktree location
      // Worktree path: .juict-agentic-os/worktrees/tasks/<worktreeName>/.juict-agentic-os/specs/<taskId>
    ...
    it('should skip non-existent task gracefully', async () => {
      ...
      // Create .juict-agentic-os directory so project is recognized
      mkdirSync(path.join(TEST_PROJECT_PATH, '.juict-agentic-os'), { recursive: true });
    ...
    it('should reject path traversal attempts in taskId', async () => {
      ...
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', 'valid-task');
      mkdirSync(specsDir, { recursive: true });
  ...
  describe('unarchiveTasks - multi-location handling', () => {
    it('should unarchive task from BOTH main and worktree locations', async () => {
      ...
      const mainSpecsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '004-unarchive-test');
      mkdirSync(mainSpecsDir, { recursive: true });
  ...
  describe('cache invalidation', () => {
    it('should invalidate cache after archiveTasks', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '005-cache-test');
      mkdirSync(specsDir, { recursive: true });
    ...
    it('should return fresh data after invalidateTasksCache is called', async () => {
      const specsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '006-invalidate-test');
      mkdirSync(specsDir, { recursive: true });
  ...
  describe('getTasks - worktree deduplication', () => {
    it('should not duplicate tasks that exist in both main and worktree', async () => {
      ...
      const mainSpecsDir = path.join(TEST_PROJECT_PATH, '.juict-agentic-os', 'specs', '007-dedupe-test');
      mkdirSync(mainSpecsDir, { recursive: true });
