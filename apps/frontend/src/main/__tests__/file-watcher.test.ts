...
describe('FileWatcher concurrency', () => {
  ...
  describe('deduplication: second watch() with same specDir is a no-op', () => {
    it('should only create one FSWatcher when watch() is called twice with the same specDir while the first is still in-flight', async () => {
      const specDir = '/project/.juict-agentic-os/specs/001-task';
      const taskId = 'task-1';
  ...
  describe('supersession: watch() with different specDir replaces the in-flight call', () => {
    it('should let the second call win when the first is awaiting close()', async () => {
      const taskId = 'task-2';
      const specDir1 = path.join('/project', '.juict-agentic-os', 'specs', '001-first');
      const specDir2 = path.join('/project', '.juict-agentic-os', 'specs', '002-second');
    ...
    it('first watch() bails when pendingWatches changes to a different specDir', async () => {
      ...
      const specDir1 = path.join('/project', '.juict-agentic-os', 'specs', 'super-first');
      const specDir2 = path.join('/project', '.juict-agentic-os', 'specs', 'super-second');
  ...
  describe('cancellation: unwatch() during in-flight watch() prevents watcher creation', () => {
    it('should not create a watcher when unwatch() is called before the async gap resolves', async () => {
      const taskId = 'task-3';
      const specDir = '/project/.juict-agentic-os/specs/003-cancel';
      ...
      const specDir2 = '/project/.juict-agentic-os/specs/003-cancel-v2';
  ...
  describe('unwatchAll() cancels all pending watches', () => {
    it('should cancel pending watch() calls and clear pendingWatches', async () => {
      ...
      const specDir1 = '/project/.juict-agentic-os/specs/004a';
      const specDir2 = '/project/.juict-agentic-os/specs/004b';
      ...
      const newSpecDir1 = '/project/.juict-agentic-os/specs/004a-v2';
      ...
      const specDirFresh = path.join('/project', '.juict-agentic-os', 'specs', '004a-fresh');
  ...
  describe('getWatchedSpecDir()', () => {
    it('returns the specDir that was passed to watch()', async () => {
      const taskId = 'task-5';
      const specDir = path.join('/project', '.juict-agentic-os', 'specs', '005-specdir');
    ...
    it('returns updated specDir after re-watch with different specDir', async () => {
      ...
      const specDir1 = path.join('/project', '.juict-agentic-os', 'specs', '005b-first');
      const specDir2 = path.join('/project', '.juict-agentic-os', 'specs', '005b-second');
