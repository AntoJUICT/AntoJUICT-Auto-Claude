...
export function getBundledSourcePath(): string {
  ...
  for (const p of possiblePaths) {
    ...
    const markerPath = path.join(p, 'runners', 'insights_runner.py');
  ...
  const fallbackMarker = path.join(fallback, 'runners', 'insights_runner.py');
...
export function getEffectiveSourcePath(): string {
  ...
  try {
    ...
    if (existsSync(settingsPath)) {
      ...
      if (settings.autoBuildPath && existsSync(settings.autoBuildPath)) {
        // Validate it's a proper backend source (must have runners/spec_runner.py)
        const markerPath = path.join(settings.autoBuildPath, 'runners', 'insights_runner.py');
        if (existsSync(markerPath)) {
          return settings.autoBuildPath;
        }
        // Invalid path - log warning and fall through to auto-detection
        console.warn(
          `[path-resolver] Configured autoBuildPath "${settings.autoBuildPath}" is missing runners/insights_runner.py, falling back to bundled source`
        );
      }
    }
  } catch {
  ...
  if (app.isPackaged) {
    ...
    const overrideMarker = path.join(overridePath, 'runners', 'insights_runner.py');
